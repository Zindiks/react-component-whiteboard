...
export const SpotifyWidget: React.FC<SpotifyWidgetProps> = ({ 
  ...
  const getSpotifyEmbedUrl = (inputUrl: string): string | null => {
    ...
    const embedRegex = /(?:https?:\/\/)?(?:open\.)?spotify\.com\/embed\/([a-z]+)\/([a-zA-Z0-9]+)(?:[?].*)?$/;
    ...
    >
      ...
      <div className="flex items-center justify-between p-2 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-sm font-semibold text-gray-800">Spotify</h3>
        </div>
      ...
      <div className="flex-grow relative">
        {isEditing ? (
          <div className="p-4 h-full flex flex-col">
            <label className="text-sm text-gray-600 mb-2">Enter Spotify URL:</label>
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-100">
            <p className="text-gray-500">No track set. Click the edit button to add a Spotify URL.</p>
          </div>
        )}
      </div>
    </div>
